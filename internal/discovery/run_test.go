...
func Test_filterNyaaFeed(t *testing.T) {
	...
	t.Run("empty", func(t *testing.T) {
		got := filterRelevantResults(animelist.Entry{}, []parser.ParsedNyaa{}, tags.Zero, newFilterData(), Config{})
		require.Empty(t, got)
	})
	...
	t.Run("airing: with latestTag", func(t *testing.T) {
		...
		got := filterRelevantResults(entry, parsedTorrents, latestTag, newFilterData(), Config{})
	...
	t.Run("airing: with repeated tag", func(t *testing.T) {
		...
		parsed := parseResults(animelist.Entry{}, input)
		got := filterRelevantResults(animelist.Entry{}, parsed, tags.SeasonEpisode(3, 1), newFilterData(), Config{})
	...
	t.Run("airing: with latestTag and quality", func(t *testing.T) {
		...
		parsed := parseResults(animelist.Entry{}, input)
		got := filterRelevantResults(animelist.Entry{}, parsed, tags.SeasonEpisode(3, 2), newFilterData(), Config{})
	...
	t.Run("aired: with batch, no latestTag", func(t *testing.T) {
		...
		parsed := parseResults(animelist.Entry{}, input)
		got := filterRelevantResults(newEntry(animelist.AiringStatusAired), parsed, tags.Zero, newFilterData(), Config{})
	...
	t.Run("aired: with batch, with latestTag", func(t *testing.T) {
		...
		parsed := parseResults(animelist.Entry{}, input)
		got := filterRelevantResults(newEntry(animelist.AiringStatusAired), parsed, tags.SeasonEpisode(3, 2), newFilterData(), Config{})
